...
[state.Population,state.Score,state.Rank,state.Distance,state.C,state.Ceq, ...
 state.isFeas,state.maxLinInfeas] = rankAndDistance(state.Population,state.Score, ...
...
for pop = 1:length(totalPop)
    ...
    [state.Population(subpopIndex,:),state.Score(subpopIndex,:), ...
        ...
        rankAndDistance(population,score,C,Ceq,isFeas,maxLinInfeas,options,popSize);
