...
if length(options.PopulationSize) > 1
    [state.Population,state.Score,state.Rank,state.Distance,state.C,state.Ceq, ...
     state.isFeas,state.maxLinInfeas] = rankAndDistance(state.Population, ...
...
if exitFlag ~= -1
    state.Score = fcnvectorizer(state.Population,FitnessFcn,size(state.Score,2),options.SerialUserFcn);
end
