    ...
    case 'iter'
        ...
        if  javaMethodEDT('getChangedState',optimtoolGui) || modelCheck
            ...
            try
                ...
                optnew = addOptimguiOutputFcn(optnew,'patternsearch');
                % Merge new and old options structures
                optnew = psoptimset(optlast,optnew);
                % Validate option
                checkoptions(optnew,psoptimset(@patternsearch),numel(optimval.x));
                updateoptions = true;
            catch ME
